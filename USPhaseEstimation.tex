...
\begin{abstract}
Ultrasound is an effective tool for rapid non-invasive assessment of cardiac
structure and function. For many diagnostic applications knowledge of the
cardiac and/or respiratory cycle phase of each frame in the ultrasound video is
essential. Accurate phase estimation typically requires auxiliary ECG or
pulseoximetry data, however, for many applications it is desirable to estimate
the phase from the image data alone. This is a particularly challenging task
particularly in pre-clinical studies involving small animals with high heart
and respiration rates. In this paper, we present a novel method for estimation
of the instantaneous cardiac and respiratory phases directly from cardiac
ultrasound videos. The method transforms the high-dimensional image sequence
into a univariate time series with the same periodicity characteristics,
decouples the periodic sources of beating heart and respiratory motion using a
trend extraction technique, and estimates the cardiac and respiratory phases
using the Hilbert transform. The proposed phase estimation is combined
with a robust non-parametric regression technique for gating out respiratory
frames for temporal super-resolution that can reconstruct high-quality images
at any cardiac phase.  We validate our methods using 2D cardiac ultrasound or
echocardiography videos and electrocardiogram (ECG) recordings of 6 mice.  
\end{abstract}
